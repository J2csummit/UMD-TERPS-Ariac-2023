  ...
  const std_msgs::msg::String::ConstSharedPtr msg)
{
  ...
  for (u_int8_t i = 0; i < parts.size(); i += 6)
  {
    if (past_placed_parts_.empty() || (std::find(past_placed_parts_.begin(), past_placed_parts_.end(), MyFloatToInt(parts.at(i+2))) == past_placed_parts_.end())){
      ...
      {
        retrieved_part.pose.position.z = parts.at(i+5) + pick_offset_ - battery_grip_offset_ + 0.0158;
      }
      else
      {
...
bool GroupCompetitor::CompleteKittingOrder()
{
  ...
  if(!order_veyor_parts_.empty() && !interrupt_){
      ...
      if (order_veyor_parts_.front().pose.position.y < -2.1){
        ...
        if(!interrupt_){
          ...
          if(floor_gripper_state_.attached){
            ...
            if (!pri){
              past_placed_parts_.push_back(belt_tray_positions_.at(part_number));
            }
    ...
    if(!interrupt_){
      ...
      if(fault_part){
        auto bad_part_pose = FrameWorldPose("agv" + std::to_string(current_order_.kitting_task.agv_number) + "_tray");
      ...
      worked_parts_count_ += 1;
      if (!pri){
        past_placed_parts_.push_back(part_tray_positions_.at(part_number));
      }
